...
class PredictCandidate:
    """One proposal from top-N generation with normalized confidence."""
...
class Predictor:
    ...
    def __call__(self, context: str) -> PredictResult:
        ...
        """
        top = self.predict_n(context, n=1, attempts=1, temperature=self.temperature)
        if not top:
            return PredictResult(text="", ids=[], mean_logprob=float("-inf"))
        best = top[0]
        return PredictResult(text=best.text, ids=best.ids, mean_logprob=best.mean_logprob)
    ...
    def predict_n(
        ...
        *,
    ...
    ) -> List[PredictCandidate]:
        """
        Generate up to `n` unique proposals and return them sorted by score.

        Scores are length-normalized (`mean_logprob`) and then converted to
        normalized probabilities with softmax over the returned candidate set.
        """
        ...
        for _ in range(attempts):
            ...
            if len(unique) >= n and attempts == n:
                break
        ...
        ranked = sorted(unique.values(), key=lambda c: c.mean_logprob, reverse=True)[:n]

        max_lp = max(c.mean_logprob for c in ranked)
        denom = sum(math.exp(c.mean_logprob - max_lp) for c in ranked)
        for c in ranked:
            c.normalized_prob = math.exp(c.mean_logprob - max_lp) / denom if denom > 0 else 0.0

        return ranked

    @torch.no_grad()
    def _score_generated(self, src_tensor: torch.Tensor, gen_ids: List[int]) -> float:
        """Teacher-forced rescoring for one generated sequence."""
